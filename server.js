...
app.put('/api/rename',passport.authenticate('jwt', { session: false }), async (req, res) => {
  ...
  try {
    ...
    await db.none('UPDATE webtech.files SET filename = $1 WHERE filename = $2', [newFilenameWithExtension, oldFilename]);

    res.json({ message: `File "${oldFilename}" renamed to "${newFilenameWithExtension}" successfully!`});
  } catch (error) {
    res.status(404).json({ error: 'File not found' });
  };
});
